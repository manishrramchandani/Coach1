...
Requires macOS + Xcode 15+. The project is generated from `project.yml` with [XcodeGen](https://github.com/yonsm/XcodeGen) (standard iOS practice — avoids a hand-maintained `.pbxproj`):
...
The app launches straight into the seeded client ("Sam", coached by "Maya") so every screen has data. Toggle **Account → Offline mode** to prove offline logging + later sync.
